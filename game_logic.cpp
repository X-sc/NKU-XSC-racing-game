...
// ====================== 全局贴图变量 ======================
Texture2D carTextures[CAR_COUNT];          // 3辆玩家车辆贴图
Texture2D obstacleTextures[5];             // 5种障碍物贴图
Texture2D itemTextures[7];                 // 7种道具贴图
...
static void LoadAllTextures() {
    ...
    // 加载障碍物贴图
    obstacleTextures[0] = LoadTexture("res/texture/cat.png");
    obstacleTextures[1] = LoadTexture("res/texture/people.png");
    obstacleTextures[2] = LoadTexture("res/texture/bike.png");
    obstacleTextures[3] = LoadTexture("res/texture/bus.png");
    obstacleTextures[4] = LoadTexture("res/texture/static.png");

    // 加载道具贴图
    itemTextures[ITEM_LIFE] = LoadTexture("res/texture/life.png");
    itemTextures[ITEM_SCORE] = LoadTexture("res/texture/score.png");
    itemTextures[ITEM_SPEED] = LoadTexture("res/texture/speed.png");
    itemTextures[ITEM_SHIELD] = LoadTexture("res/texture/shield.png");
    itemTextures[ITEM_SLOW] = LoadTexture("res/texture/slow.png");
    itemTextures[ITEM_NITRO] = LoadTexture("res/texture/nitro.png");
    itemTextures[ITEM_MAGNET] = LoadTexture("res/texture/magnet.png");

    // 加载特殊元素贴图
    ghostTexture = LoadTexture("res/texture/ghost.png");
    puddleTexture = LoadTexture("res/texture/puddle.png");
    roadblockTexture = LoadTexture("res/texture/roadblock.png");
    glareTexture = LoadTexture("res/texture/glare.png");
}
...
static void SpawnGhostObstacle() {
    for (int i = 0; i < GHOST_OBSTACLE_MAX; i++) {
        if (!g_ghostObsList[i].active) {
            ...
            // 设置幽灵障碍物尺寸
            if (r == 0) { g_ghostObsList[i].w = 42; g_ghostObsList[i].h = 52; }
            else if (r == 1) { g_ghostObsList[i].w = 52; g_ghostObsList[i].h = 105; }
            else if (r == 2) { g_ghostObsList[i].w = 52; g_ghostObsList[i].h = 72; }
            else if (r == 3) { g_ghostObsList[i].w = 82; g_ghostObsList[i].h = 92; }
            else { g_ghostObsList[i].w = 62; g_ghostObsList[i].h = 105; }
...
static void SpawnDynamicObs() {
    for (int i = 0; i < MAX_OBSTACLE; i++) {
        if (!g_obsList[i].active) {
            ...
            // 设置障碍物尺寸
            if (r == 0) { g_obsList[i].w = 48; g_obsList[i].h = 60; }
            else if (r == 1) { g_obsList[i].w = 100; g_obsList[i].h = 200; }
            else if (r == 2) { g_obsList[i].w = 60; g_obsList[i].h = 85; }
            else { g_obsList[i].w = 95; g_obsList[i].h = 105; }
...
void GameDraw() {
    BeginDrawing();
    ClearBackground(g_isNitroActive ? BLACK : DARKGRAY);
    ...
    // 主游戏界面绘制
        ...
        ClearBackground(g_isNitroActive ? BLACK : (g_currentWeather == WEATHER_FOGGY ? DARKGRAY : LIGHTGRAY));
        ...
        // 绘制道路和车道线
        DrawRectangle((int)(ROAD_X + g_shakeOffsetX), (int)g_shakeOffsetY, ROAD_WIDTH, SCREEN_HEIGHT, g_isNitroActive ? DARKGRAY : GRAY);
        DrawRectangle((int)(ROAD_X + g_laneWidth - 2 + g_shakeOffsetX), (int)g_shakeOffsetY, 4, SCREEN_HEIGHT, g_isNitroActive ? YELLOW : WHITE);
        DrawRectangle((int)(ROAD_X + g_laneWidth * 2 - 2 + g_shakeOffsetX), (int)g_shakeOffsetY, 4, SCREEN_HEIGHT, g_isNitroActive ? YELLOW : WHITE);
